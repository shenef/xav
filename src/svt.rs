...
fn decode_chunks(
    ...
    match (low_mem, inf.is_10bit) {
        (true, true) => dec_10bit(&filtered_chunks, source, inf, tx),
        (true, false) => dec_8bit(&filtered_chunks, source, inf, tx),
        (false, true) => dec_10bit_full(&filtered_chunks, source, inf, tx),
        (false, false) => dec_8bit_to_10bit(&filtered_chunks, source, inf, tx),
    }
...
fn write_frames(
    ...
    let mut written = 0;
    let mut buf = if low_mem { Some(vec![0u8; calc_10bit_size(inf)]) } else { None };
...
fn proc_chunk(
    ...
    let written = write_frames(&mut child, data.frames, config.inf, config.low_mem);
...
pub fn encode_all(
    ...
    let (tx, rx) = bounded::<ChunkData>(args.worker.min(1));
    let rx = Arc::new(rx);
    ...
    let decoder = {
        ...
        let inf = inf.clone();
        let low_mem = args.low_mem;
        thread::spawn(move || decode_chunks(&chunks, &idx, &inf, &tx, low_mem, &skip_indices))
    };
    ...
    for _ in 0..args.worker {
        ...
        let prog = prog.clone();
        let ctx = WorkerCtx { crf, low_mem: args.low_mem, quiet: args.quiet };
